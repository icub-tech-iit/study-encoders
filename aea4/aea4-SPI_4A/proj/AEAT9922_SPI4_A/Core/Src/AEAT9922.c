 ...
 * @note   This function executes the two-transaction write sequence.
 ...
 * @note   This function implements the pipelined read sequence by using
...
HAL_StatusTypeDef AEAT9922_ReadPosition_SPI4A(AEAT9922_Reading_t *reading) {
    ...
    // Master sends frame: [P | RW=1 | 000000 | 0x3F]
	...
	//The position data-frame is 18bits by default, 20bit with diagnostic. 24 bits are received, thus 4 bits are discarded.
	received_frame = ((uint32_t) rx_buf[0] << 16) | ((uint32_t) rx_buf[1] << 8) | ((uint32_t) rx_buf[2]);
	reading->raw_frame = received_frame >> 4;
	//Sectioning the received data-frame according to application note
	...
	// Here the crc field is used for parity check
